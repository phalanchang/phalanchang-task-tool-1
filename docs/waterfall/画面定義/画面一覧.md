...
|-----|--------|-------------|-----|----------|
| 1 | ダッシュボード画面 | `frontend/src/pages/Dashboard.tsx` | `/dashboard` | デイリータスク統計・チャート表示、週間分析 |
...
- **URL**: `/dashboard`
- **目的**: デイリータスクの包括的な統計分析とモニタリング
- **機能**:
  - **今日の統計**: 完了率、獲得ポイント、未完了タスク数表示
  - **週間チャート**: 日別タスク完了状況のバーチャート（Recharts使用）
  - **先週比較**: 完了率の前週比較と改善トレンド表示
  - **タスク別分析**: 個別タスクの成功率と7日間実行履歴
  - **リアルタイム更新**: 手動リフレッシュ機能
