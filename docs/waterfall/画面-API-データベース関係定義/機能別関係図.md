...
| 16 | メモ一覧表示 | GET /api/memos | memos | memos → API → 画面表示 |
| 17 | メモ作成 | POST /api/memos | memos | 画面入力 → API → memos INSERT |
| 18 | メモ更新 | PUT /api/memos/:id | memos | 画面入力 → API → memos UPDATE |
| 19 | メモ削除 | DELETE /api/memos/:id | memos | 画面操作 → API → memos 論理削除 |
...
- **画面**: `MemoPage.tsx`
- **API**: `GET /api/memos`
- **Controller**: `memosController.getAllMemos()`
- **Model**: `Memo.findAll()`
- **データベース**: `memos`テーブル
- **条件**: `WHERE user_id = ? AND is_deleted = false ORDER BY created_at DESC`

**データフロー**:
```
MemoPage.tsx → fetchMemos() → GET /api/memos → Memo.findAll() → memos table
```

### 3-2. メモ作成機能
- **画面**: `MemoPage.tsx` → `MemoForm.tsx`（モーダル）
- **API**: `POST /api/memos`
- **Controller**: `memosController.createMemo()`
- **Model**: `Memo.save()`
- **データベース**: `memos`テーブル（INSERT）

**データフロー**:
```
MemoForm.tsx → handleFormSubmit() → POST /api/memos → Memo.save() → INSERT memos table
```

### 3-3. メモ更新機能
- **画面**: `MemoPage.tsx` → `MemoForm.tsx`（モーダル）
- **API**: `PUT /api/memos/:id`
- **Controller**: `memosController.updateMemo()`
- **Model**: `Memo.save()`
- **データベース**: `memos`テーブル（UPDATE）

**データフロー**:
```
MemoForm.tsx → handleFormSubmit() → PUT /api/memos/:id → Memo.save() → UPDATE memos table
```

### 3-4. メモ削除機能
- **画面**: `MemoPage.tsx` → `MemoCard.tsx`
- **API**: `DELETE /api/memos/:id`
- **Controller**: `memosController.deleteMemo()`
- **Model**: `Memo.delete()`
- **データベース**: `memos`テーブル（論理削除）

**データフロー**:
```
MemoCard.tsx → handleDeleteMemo() → DELETE /api/memos/:id → Memo.delete() → UPDATE memos table (is_deleted = true)
```
...
3. **メモ管理**: `memos` → **独立** → 直接CRUD操作
4. **ポイント表示**: `user_points` + `point_history` → **計算** → 表示
5. **自動処理**: スケジューラー → **日次生成** + **日付変更検知**
