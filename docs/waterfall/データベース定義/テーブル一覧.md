...
| 5 | `memos` | メモデータ | メインテーブル | id, user_id, title, content, is_deleted | なし |
...
| memos | idx_user_id | user_id | ユーザー別メモ取得 |
| memos | idx_created_at | created_at | 作成日時順ソート |
...
### 5. memos（メモテーブル）
- **目的**: ユーザーが作成したメモデータを格納
- **種類**: メインテーブル
- **レコード数**: 可変（ユーザー作成メモ）
- **特徴**: 論理削除対応（is_deletedフラグ）
...
- `memos.user_id`: ユーザー別メモ取得で使用
- `memos.created_at`: 作成日時順ソートで使用
