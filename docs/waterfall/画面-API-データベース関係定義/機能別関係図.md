...
|-----|-----------|---------|-------------|-----------------|
| 1 | ダッシュボード統計表示 | GET /api/daily-task-stats | tasks, recurring_tasks | 統計データ取得・チャート表示 |
...
### 画面機能: デイリータスク統計・モニタリング
- **画面**: `Dashboard.tsx`
- **Hook**: `useDailyTaskStats.ts`
- **API**: `GET /api/daily-task-stats`
- **Controller**: `dailyTaskStats.js`
- **データベース**: `tasks`, `recurring_tasks`テーブル
- **条件**: 
  - 今日: `WHERE is_recurring = TRUE AND DATE(created_at) = TODAY`
  - 今週: `WHERE is_recurring = TRUE AND created_at >= WEEK_START AND created_at <= WEEK_END`
  - 先週: `WHERE is_recurring = TRUE AND created_at >= LAST_WEEK_START AND created_at <= LAST_WEEK_END`
  - タスク別: `JOIN recurring_tasks ON source_task_id`

**データフロー**:
```
Dashboard.tsx → useDailyTaskStats.ts → GET /api/daily-task-stats → 
  dailyTaskStats.js → multiple pool.execute() → 
    tasks table (today stats) →
    tasks table (weekly stats) →
    tasks table (last week stats) →
    tasks + recurring_tasks JOIN (task breakdown) →
  Response JSON → Recharts visualization
```

### 統計データ構造
- **今日の統計**: 総タスク数、完了数、完了率、獲得ポイント
- **週間統計**: 日別完了データ配列、週間合計、週間完了率
- **先週比較**: 完了率の差分、改善/悪化判定
- **タスク別分析**: 成功率、連続実行日数、7日間履歴
